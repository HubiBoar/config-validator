...
public sealed class IsValid<T> : OneOfBase<Valid<T>, InValid<T>>, IValidatable
    where T : IValidatable
{
    ...
    {
        return Match<OneOf<Success, ValidationErrors>>(valid => new Success(), inValid => inValid.ValidationErrors);
    }
}